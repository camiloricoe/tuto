...
export default async function ProgramGroupsPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Grupos del programa</h2>
        {canWrite && (
          <Button asChild>
      ...
      <div className="grid gap-3">
        {groups.map((group) => {
          ...
          const statusClass =
            STATUS_CLASSES[group.status] ?? 'bg-muted text-muted-foreground'
          return (
            <Card key={group.id} className="glass-subtle">
              <CardContent className="flex items-center justify-between gap-4 py-4">
                ...
                <Button asChild variant="outline" size="sm">
                  <Link href={`/a/academic/groups/${group.id}`}>Abrir</Link>
                </Button>
              </CardContent>
            </Card>
          )
        ...
        {groups.length === 0 && (
          <p className="text-muted-foreground">No hay grupos para este programa.</p>
        )}
      </div>
    </div>
  )
