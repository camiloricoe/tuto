...
export default defineConfig({
  ...
  projects: [
    ...
    {
      name: 'super-admin',
      testMatch: /smoke\.spec\.ts|admin-chained\.spec\.ts|feedback\.spec\.ts|tenant-isolation\.spec\.ts|audit-verification\.spec\.ts|crud-validation\.spec\.ts|integrations\.spec\.ts|chaos\.spec\.ts|security\.spec\.ts|data-flow\.spec\.ts/,
      use: { ...devices['Desktop Chrome'], storageState: 'tests/e2e/.auth/super-admin.json' },
      dependencies: ['setup'],
    },
    {
      name: 'admin',
      testMatch: /per-role\.spec\.ts|permissions\.spec\.ts|role-data-flow\.spec\.ts/,
